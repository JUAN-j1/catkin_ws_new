...
void goals_a(MoveBaseClient &ac, float x1, float y1) 
{	
	...
	ROS_INFO("Sending goal to robot_a %f %f",goal.target_pose.pose.position.x,goal.target_pose.pose.position.y);
	ac.sendGoal(goal);
...
void goals_b(MoveBaseClient &ac_b) 
{
	...
	ROS_INFO("Sending goal to robot_b %f %f",goal_b.target_pose.pose.position.x,goal_b.target_pose.pose.position.y);
...
void goals_c(MoveBaseClient &ac_c) 
{
	...
	ROS_INFO("Sending goal to robot_c %f %f",goal_c.target_pose.pose.position.x,goal_c.target_pose.pose.position.y);
...
void goals_d(MoveBaseClient &ac_d) 
{
	...
	ROS_INFO("Sending goal to robot_d %f %f",goal_d.target_pose.pose.position.x,goal_d.target_pose.pose.position.y);
...
ROS_INFO("Waiting for the move_base action server to come up 3");
}
...
// Timanfaya (x=785, y=785) Rio Tinto (x=1361 y=787) La cabrera: (x=715, y=715)
x1_coord=715-2*x1_coord;
y1_coord=715-2*y1_coord;
...
{
if((ac.getState() == actionlib::SimpleClientGoalState::SUCCEEDED)  )  //&& (distance_a_b( pos_x, pos_y, b_pos_x, b_pos_y)==0)
{
xant=x1; yant=y1;
x1=x3; y1=y3; x2=x4; y2=y4;
conta_aux_2=fscanf(fp,"%d\t%f,%f\t%f,%f",&index,&x3,&y3,&x4,&y4);
ROS_INFO("New goal robot");
}
}
//////////////////////////////////////////////////////////////////////
//robot 
...
ROS_INFO("Sending goal: %f,%f,%f,%f %f . ALso: %d",x1-ox1,y1-oy1,x3-ox1,y3-oy1, 360*(atan((y3-y1-oy1)/(x3-x1-ox1)))/(2*PI) ,conta_aux);
...
else
{
	...
	{
		//ROS_INFO("distancia entre a y b :%f",distance_a_b( pos_x, pos_y, b_pos_x, b_pos_y));
		//ROS_INFO("distancia entre b y c: %f",distance_b_c( b_pos_x, b_pos_y, c_pos_x, c_pos_y));
		//ROS_INFO("distancia entre c y d: %f",distance_c_d( c_pos_x, c_pos_y, d_pos_x, d_pos_y));
...
if( (ac.getState() == actionlib::SimpleClientGoalState::SUCCEEDED) ) //&& (distance_a_b==0)
{
